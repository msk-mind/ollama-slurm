...
CONFIG_DIR="${REPO_ROOT}/model_configs"
...
while [[ $# -gt 0 ]]; do
  ...
  --help | -h)
    ...
    echo "  --email EMAIL       Email address for notification (env: NOTIFY_EMAIL)"
    echo "  --ntfy-topic TOPIC  Ntfy topic for push notifications (env: NTFY_TOPIC, default: llama-servers)"
    echo "  --ntfy-server URL   Ntfy server URL (env: NTFY_SERVER, default: https://ntfy.sh)"
...
EXPORT_VARS="ALL,MODEL_FILE=${MODEL_FILE},CONTEXT_SIZE=${CONTEXT_SIZE},N_GPU_LAYERS=${N_GPU_LAYERS},SCRIPT_DIR=${REPO_ROOT}"
[ -n "$EXTRA_ARGS" ] && EXPORT_VARS="${EXPORT_VARS},EXTRA_ARGS=${EXTRA_ARGS}"
[ -n "$NOTIFY_EMAIL" ] && EXPORT_VARS="${EXPORT_VARS},NOTIFY_EMAIL=${NOTIFY_EMAIL}"
[ -n "$NTFY_TOPIC" ] && EXPORT_VARS="${EXPORT_VARS},NTFY_TOPIC=${NTFY_TOPIC}"
[ -n "$NTFY_SERVER" ] && EXPORT_VARS="${EXPORT_VARS},NTFY_SERVER=${NTFY_SERVER}"
[ -n "$REGISTRY_URL" ] && EXPORT_VARS="${EXPORT_VARS},REGISTRY_URL=${REGISTRY_URL}"

JOB_ID=$(sbatch ${SBATCH_OPTS} \
  --cpus-per-task="${CPUS}" \
  --mem="${MEM}" \
  --gres="${GPU_GRES}" \
  --output="${REPO_ROOT}/llama_server_%j.log" \
  --error="${REPO_ROOT}/llama_server_%j.err" \
...
echo "  ./connect_claude_llama.sh $JOB_ID"
echo ""
